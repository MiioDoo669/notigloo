...
while True:
    ...
    if landmark_points:
        ...
        dot_color = (0, 255, 0)
        if eye_distance < 0.01: dot_color = (0, 0, 255)
        eye_indices = [159, 145, 133, 33, 7, 163, 144, 153, 154, 155]
        for idx in eye_indices:
            px = landmarks[idx]
            cv2.circle(frame, (int(px.x * frame_w), int(px.y * frame_h)), 2, dot_color, -1)
        ...
        #  SYNC LOGIC 
        if eye_distance < 0.01: 
            if blink_start_time == 0: blink_start_time = time.time()
            duration = time.time() - blink_start_time
            
            if duration > drowsy_limit:
                if not video_playing:
                    pygame.mixer.music.play(-1) 
                    video_start_clock = time.time() 
                    video_playing = True
                    drowsy_events += 1
                
                
                elapsed = time.time() - video_start_clock
                frame_id = int(elapsed * fps)
                
                cap_v.set(cv2.CAP_PROP_POS_FRAMES, frame_id)
            ...
            blink_start_time = 0
            if video_playing:
                try: cv2.destroyWindow('EMERGENCY ALARM VIDEO')
                except: pass
